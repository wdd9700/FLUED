...
It is intentionally separate from `FLUED_V3_CONTROL.md` so that future context
compression does not erase the current reasoning state.

Current handoff note:

```text
Read `FLUED_V3_1_REVIEW_AND_NEXT_STEPS.md` first after any context compression.
...
Then read `FLUED_V3_1_ARCHITECTURE_CN.md` for the current Chinese architecture
...
```text
tools/analysis/train_v3_commit_controller_small.py
```
...
```text
tools/analysis/train_v3_segmental_workspace_2m.py
```
...
```text
tools/launcher/run_v3_segmental_workspace_2m_matrix.ps1
```

ROI / latent-state visualizer:

```text
tools/eval/eval_v3_segmental_workspace_roi.py
```
...
```text
tools/analysis/train_v3_segmental_diffusion_2m.py
```
