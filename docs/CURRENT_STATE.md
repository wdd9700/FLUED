...
| E37 | **k=1 主干接口在公平步数（20K）下分数可能略低、但上限更高**（用户预测，2026-08-06）——可证伪测量：s20 vs s17（及 E30/s16 参照）milestone 评测轨迹（3K-20K 七点、同一评测流同一 mask，`eval_s1_milestones.py`）；判决读数 = 末段斜率（上限/饱和信号）+ 同分点对位（当前进步速度）；终评按规格 §21 预注册线（接近 s17 强阳性 / ≥0.5 前端解锁 / ~0.2 读出包容量瓶颈） | 候选（测量臂 s20 在跑） | 预注册 2026-08-06 |
