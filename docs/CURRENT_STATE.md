...
| E35 | **健康体制（latent 预测）2×2 矩阵**（主干 × 状态通道，其余全同 v36.7）：attn+有状态 s16 = 0.832/0.785/2.30/cos 0.903；attn+无状态 s18 = 0.850/0.805/2.12/cos 0.916；mlp+有状态 s17 = 0.843/0.799/2.17/cos 0.953（state_norm 8.7 全程最健康）；mlp+无状态 s19 在跑。两读数：① **attn 下无状态通道全胜有状态（+2pp 级）——双向注意力存在时串行状态压缩只丢信息**；② mlp 下 predict_cos 反而全场最高（0.953，无近道可抄的诚实预测）——状态通道的真实价值必须在"它是唯一跨段通道"的形态（mlp）里度量，s17 vs s19 即 R1 的正式判决格 | 已验证（单 seed，20K/512B 工程口径；s19 格待补） | s16/s17/s18（2026-08-06） |
