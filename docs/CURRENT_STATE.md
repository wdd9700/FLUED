...
- 2026-08-05：**S0′′ 扩量落地**：8K 标注完成（6,572/8,000 = 82.2% 合格率，与 K2.5
  轮 81.8% 完全同水平；失败主因异型文本字符增删，非 API 问题）；合并 K2.5 锚 +
  pro 增量经 >21字/>9词硬上限预筛得 **7,226 条**（k25 1,285 + deepseek_v4pro
  5,941，来源字段已落）；预筛同时剔出 K2.5 锚自身 401 条（上轮 QA"残余偏粗
  +14%"的实锤）。合并后粒度：k25 24.8B/p50 22B vs pro 25.3B/p50 24B——对齐。
  S0′′ 从零 SFT 启动（`checkpoints/s0pp_pro_v4_sft_20260805`，超参同 S0′，18
  epochs）；failures 返工轮（1,428 条）并行进行，回收后重合并入档。
