...
| E39 | **s21（d_backbone 1536 对齐 d_pack，124.3M）：读出加宽零效应**——backbone 0.2060 vs s20 的 0.2059（四位有效数字相同），direct 0.816（+3pp 宽通道红利）/state_norm 3.3 健康；轨迹 backbone 全程平台。**"单向量宽度是掐口"假设证伪**。位置分辨探针（`probe_position_acc.py`）：backbone 按距末端距离全程平坦 ~0.2（含最终段自身 0.17），**无近因梯度**——排除"状态只存近期内容"；瓶颈判定为**单一固定读出查询对 23 次覆写状态的线性切片提取上限**（读出机制层面，非宽度、非状态容量）。下一臂：/n 翻页读出（用户许可的 nq 合法形态），n=4 首测 | 已验证（负结果+机制定位，单 seed 工程口径） | s21_k1_d1536_20k（2026-08-07） |
