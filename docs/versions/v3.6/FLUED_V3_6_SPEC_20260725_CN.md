...
> **口径注记（2026-08-07 审计 B10）**：本表为 v3.6 立线时点数值。"readout 包 k×384"
> 基于旧 d_pack=384；v36.1 起 d_pack=1536，现行配置状态 131K 标量 ≈ 包的 85 倍
> （非表内 2.6 倍口径）。k=1 容量重校随 s20 判读进行（见 §21 与 CURRENT_STATE E38）。
...
> **时态注记（2026-08-07 审计 B9）**：第 4 条已由 E33 闭环（fla chunk_kda 默认 +
> stream_step 流式原语）；第 6 条已由 v3/v4 语料全面启用事实闭环；第 5 条的
> "文档级评测工具"**至今未建**——它是状态通道判决的需求前置（D5），优先级最高。
...
> **实现偏差注记（2026-08-07，审计 B8）**：本节落地时把逐段读出一路接到了主干
> 输入（v36.2-v36.7），超出本意；§21 正本：逐段读出仅 decoder 侧任务密度手段，
> 主干接口 k=1（`backbone_readout=final`）。本节结果（E23）依然有效，但"主干
> 吃 23 段"的形态读数（E35 2×2）仅作参照。
...
> **实现偏差注记（2026-08-07，审计 B8）**：D7 的 0.549 为"逐段读出喂主干"形态
> 读数；k=1 形态下 summarizer 形态结论方向仍有效、数值需重锚（见 §21 与
> CURRENT_STATE E38）。另：S0.9 的结果节误置于本节末尾（见 §19 注记）。
...
**S0.9 结果（2026-08-05）**：前缀还原曲线（restore chunks 0..i from S_i，
分位桶 q0→q8）：**0.273 → 0.196，仅 −7.7pp 平缓衰减**——KDA 状态对早期内容
保持力良好、无悬崖褪色（状态保持力的首个直接证据，CBIU 逐位置效用仪器落地）。
三角对照：q8（单状态还原全前缀 0.196）< D7 逐段读出 0.549（逐段条件化价值的
另一侧量化）> A0 均值通道 0.154（位置化条件优于全局均值）。口径记录：S0.9
训练损失全部来自前缀任务，标准逐段路径仅作指标输出（未受训），其读数 0.210
不与 D7 同任务可比。工程记录：训练中途完成效率优化（冻结前端 no_grad +
训练期标准路标度免梯度 + fla chunk_kda 融合核备选 parity 1e-3 级），
S0.9 全程数值语义未变。归档 `L:\FLUED_archive\s09_prefix_20k_20260805`。

## 19. 2026-08-05 预注册：S0.9 流式前缀还原任务（用户设计，裁决采纳）

> **结构注记（2026-08-07，审计 B7）**：S0.9 的结果节历史上误写在 §18 末尾
> （前缀曲线 0.273→0.196 平缓衰减，状态保持力良好），阅读时以该处为准；
> 后续翻页曲线延长（64/128 段、分域）属 D6 待办。

**任务重定义（用户裁决）**：decoder 在第 i 段吃当时 KDA 状态 S_i 的读出，
还原**截止到第 i 段的全部字节**（有多少信息还原多少内容），替代"只还原本段"。
原任务自动成为 i=C 的特例。设计意图（用户原话要点）：① 保证 decoder 精准还原
所有已编码字节的能力；② 更好地监督主干——主干不得破坏全局语义、只补全 mask
字节（mask 散落在全上下文）；③ 为生成式任务做根本性适应——任意 ≤512B 内容
可独立成段/续写；④ 为 1024/2048/4096 的 scaling 试探铺路。训练密度 ~20×
（每段一个监督点），用户明确裁决密度即目的、O(C²/2) decoder 开销接受
（decoder 仅 6.5M）。

**实现口径**：沿用 D7 组合（DiT summarizer + per_chunk_readout）；第 i 位
条件向量 = S_i 读出 + chunk_pos，还原段 1..i；mask 协议不变（5%、span 1-8、
先 mask 后编码）；masked 补全目标=前缀内被遮字节。训练期采样前缀位置子集
（含终点 i=C 必采），eval 全位置。
**协议**：从零消融（只接管 S0 四前缀）、20K 步、同数据/种子/eval。
**判定**（对照 D7：0.5491/0.1428/5.81）：主指标全位置 acc 与 PPL（≥D7 即
新任务不劣），**新增前缀还原曲线（acc vs 段位置）作为主交付**——曲线形状
回答"状态信息随段数累计/褪色"的保持力问题（CBIU 逐位置效用的原生仪器）；
masked 为必备观察项。守卫照旧。归档 `L:\FLUED_archive\s09_prefix_20k_20260805`。

## 20. 2026-08-05 预注册：S1.0 三任务拆分（decoder/主干彻底分工）

> **实现偏差注记（2026-08-07，审计 B8）**：本节三任务在 v36.2-v36.7 的实现中
> 主干吃全部 C 个逐段读出；§21 正本后主干接口 k=1（最终读出），predict 改为
> 末二读出→末段单对。本节任务定义本身不变，输入形态以 §21 为准。

（编号说明：v3.6 世代 S 系列与旧文档中 "S1=seed 1" 含义不同，无冲突。）

**任务架构（用户设计，全员裁决采纳）**：

| 路径 | 输入 | decoder 目标 | 考核对象 |
|---|---|---|---|
| direct（codec 本职） | readout 原样 | as-encoded 版本（mask 输入则还原带 MASK_ID 的版本） | 纯翻译保真 |
| backbone·补全 | readout→backbone→新矩阵 | 补全后的未 mask 原文 | 主干独占：语境推断填字节 |
| backbone·预测 | readout→backbone→新矩阵 | 下一 chunk 内容 | 生成/推理的根 |

**关键纪律**：masked 补全只从主干路径计分（direct 不再考）——主干空转在结构上
被消灭。预测任务采用用户提出的快路径：**潜空间差值**——
`MSE(backbone_out[i], content[i+1].detach())`，目标侧 stop-gradient 写死
（防 JEPA 式互相靠拢坍缩；codec 任务锚定内容侧）。**裸跑**（用户裁决）：
不做差值加权/脚手架，先看曲线形状再决定是否加拐杖。

**指标口径修正（S1.0 起生效，与历史数字不可直接比）**：
- direct 保真 = as-encoded 还原率（MASK_ID 位单列，防平凡正确虚高）；
- masked acc = 仅主干路径（补全能力的净读数）；
- 预测 = predict_cos（潜空间余弦）+ 抽样 byte 级解码准确率。

**协议**：D7 条件（DiT summarizer + per_chunk_readout）+ 新任务集，从零消融
（只接管 S0 四前缀）、20K 步、同数据/种子/eval。对照 D7 与 S0.9。
**判定**：direct 保真应 ≥D7 全位置口径（任务变易，预期上升）；masked（主干）
与 D7 的 0.1428 比——若显著上升，说明"独占角色"激活了主干；predict_cos 曲线
为新增观察项。归档 `L:\FLUED_archive\s10_three_task_20k_20260805`。

**S1.0 结果（2026-08-05，裸跑）**：direct 保真（as-encoded）**0.610**、
主干补全路径 unmasked **0.597**、PPL **5.24**（三条全是历史最佳）；predict_cos
**0.896**（主干确实在学下一段内容，非空转）；masked（主干独占口径）0.138
——**独占角色没有推高 masked**（历史带宽 0.14-0.15 内）。这是"masked 上限
是任务难度而非主干容量"的第三条独立证据（前两条：direct≈backbone 打平、
HNet 瓶颈臂同水平）。注意项：state_norm 16.3 明显上行（此前 2.4-8），无 NaN，
列入稳定性观察。

**S0′ 对照读数（2026-08-05 补录，兑现 changelog 声明）**：S0′ 前端（K2.5 教师，
用户 21B 粒度）同协议下游：direct 0.772 / unmasked 0.765 / PPL 2.74 / masked 0.148，
段数 23.0（E27，byte_span 口径）——较 S1.0 旧前端全项 +16pp 级大胜。后续 S0′′
扩量（7,363 条双源教师）再推进至 direct 0.836 / PPL 2.25（E30，混合口径）。

## 21. 2026-08-06 用户裁定：设计思想正本（k=1 主干接口与语义段假设）

> 本节为用户对 v3.6 架构设计意图的权威澄清，优先级高于 §1-§20 中与之冲突的
> 实现描述。E23（§17）把逐段读出一路接到主干输入属实现偏差，本节予以纠正。

1. **逐段条件化的本意是 decoder 侧任务密度**：KDA 每消费到第 i 段即从当前
   状态出一个读出，是给 decoder/direct/补全任务每段独立条件与目标（单样本
   监督密度 ×~20）。**不是让主干面对 C 个读出**。
2. **主干接口 k=1 不变**：任意长度的前置上下文 → 整段并行切分 → 每 chunk
   并行总结（summarizer 存在的意义：把 KDA 串行更新从 512 次降到 ~23.5 次，
   量级问题）→ KDA 串行更新 → **最终状态出一个读出**给主干。要解放的是
   "512B 绑死"的长度限制（让主干适应任意长前置上下文），不是 k。
3. **读出必须含历史**：读出若只含第 i 段自身信息，KDA 即形同虚设（直接读
   memory 即可）。状态通道是定义性组件，不是可拆附件。
4. **主干任务**：① 用整段已有上下文补全被 mask 的部分（全文理解）；
   ② 生成下一 chunk 内容（段级自回归）。
5. **语义段假设**：语义段 = 最小连贯语义单元（按语言的逻辑与语言学形态
   切分），因此段级并行生成不破坏语言因果——本架构是"彻底并行扩散生成"
   与"逐字节自回归"之间的中点：并行区域局限在语义段假设成立的范围内，
   因果性不被破坏，效率提升。因果生成是已被证明强制模型产生智能的主流
   方式，本架构在其约束下求最大并行。
6. **翻页 = 存档，非丢弃**：已满的 readout 矩阵以保持最大保真度的有损形态
   归档，在不破坏序列的前提下延展上下文；transformer 全局注意力用于
   **归档页之间**的关联（档案馆模型）。
7. 实现落点：`--backbone-readout final`（v36.7 起）：主干只吃最终读出做
   全文补全；预测 = （第 C-1 读出 → 第 C 段）单对 latent MSE；direct/
   decoder 侧保持逐段。判读线（预注册）：s20 补全/重建接近 s17 逐段主干
   （0.84/0.80）为强阳性；≥0.5 说明现代前端解锁 k=1；~0.2 则读出包容量
   （d_pack/nq）是下一瓶颈。E18/E19 时代 k=1 低分（0.11-0.19）系旧前端 +
   decoder 侧同为 k=1 广播所致，不作数。

## 22. 2026-08-07 用户研究纲领：智能密度与 J-Space 解放

> 用户晨间整理的正本，原文收录。本节是 v3.6 全部实验的动机框架，优先级最高。

**核心问题**：结合 J-Space（Anthropic）、锥形神经网络、"RL 只训练一层是否足够"、
ELF（何恺明）、DSpark（DeepSeek）等多篇研究指向的同一件事——如何在**性能
（模型表现与下游任务）、计算成本、训练复杂度**三维上，围绕智能的本质与帕累托
前沿做研究。不仅仅是把语言模型"前面"（对齐语言模态）和"后面"（对齐任务风格）
的部分单独优化，更是把 J-Space 从原来模型中间几层的有限区域**解放为一整个
主干**。背后的语言观：语言的本质是一种**段状、脉冲式**的智能承载形式（与 §21.5
语义段假设同源）。

**组件角色正本**：

1. **segmentor + summarizer + KDA 状态机 = 前端模态处理**。其中 segmentor 承受
   的压力略大（切分决策全在它身上）。KDA 适合放在这里的原因：它**不负责深加工
   信息，而是有选择地做高保真的有损压缩**，确保高质量地呈现给主干——"高价值
   信息 + 高密度表达"的提取，是 codec 任务与主干任务对前端的共同约束。
2. **readout = 全局工作空间；主干 = 操纵工作空间的 J-Space 层**。这一层的容量
   从"涌现的、不可控的、较小的"（普通模型中间几层）被解放为**整个主干大小、
   可显式 scaling**——这提升的是小模型在长程任务上智能密度的 scaling 可能性。
3. **decoder = 最后的任务风格对齐**。

**资源配置推论（2026-08-07 用户裁定）**：压缩率不是硬瓶颈，**智能密度才是**。
nq（多查询读出）不算 k=1 的合法扩容——除非按 /n 方式翻页，否则容易破坏因果；
容量消融 v2 不开 nq 轴。

## 23. 2026-08-08 用户提案：三态草稿架构（latent thought 形态候选）

> 用户提案，经讨论收敛后登记为候选架构线。背景：E44（s30/s31 判别对）证明
> k=1 固定单查询读出是检索墙（缺精确通道），同时 s20 内部撕裂证据表明 KDA
> 状态保持力无问题（direct 20K 未封顶，详见 E44）。用户据此重构 latent
> thought 的形态：**输入态、思考态、输出态三态同种（都是潜矩阵），对应现有
> LLM 高层架构中原本由 token 序列承担的三个角色**。

**形态正本（用户裁定）**：

1. **输入态 = k=1 单状态矩阵读出**（§21 不变）：prompt 经前端压缩为一个读出
   矩阵，作为主干输入。它的岗位描述从"必须精确携带全部字节"改为"给思考提供
   grist"——字节级保真不是输入态的职责。
2. **思考态 = 全新矩阵上的迭代去噪**：去噪步数由主干决定，训练时以最大/最小
   步数兜底；每步去噪同时看见①原输入读出矩阵、②当前思考矩阵。
   **思考步不设监督**（用户裁定，RL  outcome 监督思想迁移）：思考不必朝输出
   状态单调逼近，允许分支与回退——**输出干净，思考可以脏**（设计公理）。
   监督只打在输出态上，经现有 S1.0 任务（direct/补全/预测）端到端回传。
3. **输出态 = 一张新矩阵，one-shot 交 decoder 并行解字节**（decoder 现有能力，
   零改造）。与主干推理-打草稿-誊写的模式同构：草稿先行，定稿一次输出。

**用户裁定：评测线不动**。主干两大任务（补全/预测）的梯度经 encoder 回流，
本身就在约束 codec 不许沦为盲目精确重建（E34 机制的健康方向）；不能提升
主干理解能力的方向不会得到梯度奖励。生成/问答类外部评测在本形态落地前
不是前置。

**待实验裁决的开放点（双方保留意见，预注册判别臂）**：

- **思考步可见范围**：臂 A（用户口径）思考步只看见 k=1 读出矩阵——架构更
  干净；臂 B（AI 口径）思考步额外可查阅 memory 行（精确通道）——预期分数
  更高。两臂同预算同协议对判，数据说话。臂 B 不是背叛 k=1 输入教义：输入
  编码仍是 k=1，争议仅在于思考过程的查阅权。
- **隐含赌注（未测轴）**：单趟前馈从叠加态取不出的信息，迭代去噪能否取出？
  容量矩阵证明"宽度无效"，但迭代提取是另一计算体制——本赌注吸收原 s32
  （迭代读出）候选臂。

**实现参数（2026-08-08 用户裁定，不再是开放项）**：T_min=1、T_max=16；可见性
两臂不分先后；去噪=主干整体换 DiT 做多轮去噪；训练任务与评测不变（主干职责
不变），仅新增"何时停止思考"机制。**停止机制实现（AI 落地选择）**：训练期 T
按 [T_min, T_max] 均匀采样（CPU 侧采样，免 GPU 同步税），模型对任意预算都要
好；推理期用确定性稳定停止（相邻思考矩阵余弦 >0.995 且 ≥T_min，上限 T_max）
——停止由主干自身动力学决定，不引入额外监督。噪声实现：思考矩阵以
高斯噪声+位置嵌入+可学习种子初始化，内容全部经条件通道进入。预测任务的因果
诚实落在**输入层**（预测 rollout 吃倒数第二读出、memory 截断到 last-1），
思考过程内部是全局修订（双向）——与部署形态一致（窗全编码后才打草稿）。
实现：`backbone_mode="draft"`（`DraftBackbone`，`flued/v36/model.py`），
`--think-steps-min/--think-steps-max/--think-visibility/--think-stop-cos`。

**首轮判别对结果（2026-08-08，E45）**：s32（臂 A 纯读出）backbone 0.1955 /
s33（臂 B +memory 行）0.2116，双双钉在 k=1 平台；B 对 A +1.6pp（4× 噪声带）
——同一精确通道在 s31 直接 cross-read 下值 +59pp，经草稿动力学只剩 ~3%。
**中心结论：无监督思考循环塌缩**——两臂 eval_think_steps 均塌缩至 2.0（训练
随机 1-16 轮、推理两轮收敛），无监督迭代循环自动退化为普通前馈，"迭代提取"
赌注在当前形态判负；架构级含义：读出是信息上限非计算上限，迭代计算造不出
输入里没有的信息；循环要不白给，必须引入逐步压力或强制预算（逐步监督违背
"思考可以脏"的分支精神已排除；候选=强制预算 T_min=T_max=16，待用户裁定）。
旁证：臂 B state_norm 0.139——memory 可查阅时状态机被降权，R1 弱状态签名
在草稿形态复现（codec 侧无恙，direct 0.821）。
