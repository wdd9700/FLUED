...
```text
tools/train/v3_3/train_v33.py                    单次训练 / 评估入口
tools/launcher/v3_3/run_v33_ablation_matrix.py   JSON 矩阵消融 runner
tools/launcher/v3_3/run_v33_ablation_matrix.ps1  Windows PowerShell 包装
tools/analysis/v3_3/summarize_v33_ablation.py    汇总 summary.json 为表格
configs/v3_3/v33_ablation_2m.json                2M 级核心消融矩阵
```
...
python tools\train\v3_3\train_v33.py `
  --config configs\v3_3\v33_no_memory_smoke.json `
  --device cuda `
  --amp
```
...
```powershell
.\tools\launcher\v3_3\run_v33_ablation_matrix.ps1 `
  -Matrix configs\v3_3\v33_ablation_2m.json `
...
```powershell
.\tools\launcher\v3_3\run_v33_ablation_matrix.ps1 -DryRun
```

汇总结果：

```powershell
python tools\analysis\v3_3\summarize_v33_ablation.py --root checkpoints\v33_2m_core
```
...
`configs/v3_3/v33_ablation_2m.json` 默认包含：
