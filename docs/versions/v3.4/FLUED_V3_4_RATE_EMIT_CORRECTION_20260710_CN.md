...
> **2026-07-14 口径修正：** 本文公式描述的是目标设计。当前 `exact` 路径实现前缀 log-det 增量，默认 `l2` 路径则是逐位置投影能量代理，并非该公式的低成本等价实现；当前 chunk 数也仍由固定预算决定。

> 历史机制验证：500 步实验只证明梯度路径可训练，不证明长期压缩有效。5K 结果显示固定 compute-cost 权重仍不能稳定控制实际 latent 数量。
