...
**2026-07-17 深夜勘误与根因修正**：原文"实例级效用映射不可学"表述不准确。
三步排查如下：

1. 怀疑离群值压制 MSE（|net|>1 占 0.51%，幅度达 20 倍标准差）→ Huber+截断重训，
   Spearman 仅从 0.095 升到 0.104，**排除离群值解释**；
2. 怀疑标签噪声 → MLP 直接拟合类均值标签，训练 MSE 仍 0.966（z 单位），
   **连类均值都拟合不了**，排除标签噪声；
3. 根因：**效用结构存在于离散因子水平空间（slot×punct桶×熵桶×anchor桶），
   连续特征的 MLP 无法重建该结构**——这是表征选择错误，不是数据问题。

修正后的结论：

- 类间方差占总方差 **74.4%**；train→test 类均值 Spearman **0.830**，类结构
  跨样本高度稳定；
- **可加因子模型（27 参数：全局均值 + 15 slot + 4 punct + 4 熵 + 4 anchor 水平）
  追平全类查找**（Spearman 0.433 vs 0.439，AUC 0.624 vs 0.625，残差方差相同）——
  效用在四个因子上近似可加分拆，无显著交互；
- slot 结构非单调：slot 3-8 与 13-15 高效用（均值 +0.07~+0.13，75-85% 为正），
  slot 1-2、9-12 近零（53-70%）——readout 存在位置/角色特化；
- L3 分配器的正确形态是**小因子模型/类查找**，不是 MLP 控制器；
- AUC 0.624 仍低于 0.70 闸门，L3 未过闸的判定**维持**。
...
**2026-07-24 增补：语境特征增补测试（语境顾问假说的窗内判决）**

用户归因（判一/判二）：memory 冗余的真正机制是**下游语境推断**截胡（补全所需
语境由 decoder+backbone 的 readout 序列注意力免费完成）；memory 唯一下游抢不走
的需求是 **encode-time LOD**（emit 预算与边界放置这两个编码时刻的不可逆决策）
对跨 chunk 语境的依赖——即"语境顾问"假说。检验方式：在 L2 数据集上补跨 chunk
语境特征（sim_prev1/max_sim_prev/sim_window_mean/readout_norm，train 分位数
分桶），同分裂同口径重训可加因子模型。

结果（工具：`tools/analysis/v3_5/dump_l2_context_features.py` +
`eval_l3_context_augmented.py`；归档 `L:\FLUED_archive\v35_l2_context_features_20260724`）：

| 模型 | Spearman | AUC | oracle regret |
|---|---:|---:|---:|
| 因子基线（复现勘误 0.433 ✓） | 0.4324 | 0.622 | 0.0399 |
| **因子+全部语境特征** | **0.4289** | **0.619** | **0.0399（不变）** |
| 因子+单语境特征（4 组） | 0.431-0.432 | 0.620-0.622 | — |
| 类均值查找（对照） | 0.4391 | 0.625 | — |

Δ = **-0.0035 Spearman / -0.0025 AUC / regret 零变化**：跨 chunk 语境特征对动作
效用**零增量**。语境顾问假说在窗内尺度判死——emit 动作效用是纯局部的类级属性，
encode-time LOD 决策不需要语境顾问，L3 的局部因子模型形态得到反向加固。
遗留边界：①语境特征基于 readout 余弦相似度族，更复杂的语境表征未测；②跨窗
（流式）语境不在本测试范围内，属另一架构提案。
