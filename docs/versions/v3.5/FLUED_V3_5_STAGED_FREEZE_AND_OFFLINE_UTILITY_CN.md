...
**latent 质量与 decoder 强度分离诊断（2026-07-17 修订）**：高重建只证明
codec-decoder 组合可用，强 decoder 可能从弱 latent 中榨出高重建。L0 因此同时
记录三条独立证据，避免把"decoder 很强"误判成"codec 很好"：

1. 高容量独立 decoder：系统重建上限；
2. **低容量 probe**（参数匹配的轻量 decoder 或冻结 readout 上的线性探针）：
   latent 本身的可读性（局部内容、顺序、距离-压缩率关系）；
3. L1 的 fresh-backbone 判定：latent 对主干任务的真实效用。

三者相互矛盾时以 L1 为准，但 2 的退化是 codec 质量的早期报警。
...
### 5.4b L3.5：闭环 rollout 过闸（2026-07-17 修订）

离线校准只证明控制器能**辨认**好动作，不证明它顺序执行时稳定：boundary/emit
一旦上线，会改变后续 chunk 长度、可见上下文、latent 分布和后续候选动作的状态
分布——即使控制器冻结，环境分布仍随其历史动作漂移（offline-to-closed-loop
covariate shift）。L3 离线过闸后必须追加闭环检验：

- 冻结 L0、L1 与控制器；
- 在完整序列上真实顺序 rollout（动作改变后续 chunk 化与 latent 序列）；
- 对照：同预算 uniform、legacy、随机策略与离线 oracle 上界；
- 报告：累计 utility/regret、预算稳定性、chunk 长度分布漂移、最坏序列退化；
- 过闸：长程指标不随 rollout 长度持续恶化，且最坏序列三风险退化有界。

离线 AUC 漂亮但闭环把自己带入未覆盖状态区域的经典失败，不允许在本项目重演。
...
### 7.1 证据两级制：工程闸门与论文证据（2026-07-17 修订）

| | 工程闸门 | 论文证据 |
|---|---|---|
| 用途 | 判断是否进入下一层 | 支撑最终公开 claim |
| 种子数 | 2 | 3-5，含均值/标准差/置信区间 |
| 训练预算 | 短程（10K-20K） | 与基线同预算的完整计划 |
| 数据 | train/eval 固定切分 | held-out test 只在方案锁定后启用 |
| 比较 | 预先写死阈值的单点判定 | 参数/FLOPs/token/readout 容量匹配，报告整条容量-性能前沿 |
| 预注册 | 阈值、切分、过闸规则**运行前**写入归档 manifest | 同左，且不允许看到结果后调整 |

所有层的"过闸"均指工程闸门；论文级 claim 只对最终保留下来的关键方案补跑，
不重复消耗算力在已淘汰分支上。
...
flowchart TD
    ...
    J -->|"是"| J2["L3.5 闭环 rollout 过闸"]
    J2 -->|"闭环漂移/退化"| K
    J2 -->|"稳定"| L["2048/4096 长上下文与真实速度"]
    D --> L
    C --> L
    L --> M{"率失真+墙钟双改善?"}
...
20. 不允许任何控制器未经闭环 rollout 过闸就部署到真实序列执行路径。
21. 不允许看到结果后调整阈值、数据切分或过闸规则；全部预注册进归档 manifest。
22. 不允许论文 claim 使用单点最优结果；必须报告容量-性能前沿与多种子置信区间。
