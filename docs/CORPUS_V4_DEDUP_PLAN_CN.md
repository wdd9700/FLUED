...
- 输出位置：`N:\FLUED_corpus_v4`
- 编码：UTF-8
- 形式：多个文本分片，整体视为一份语料
- 去重：按规范化后的文档/行内容哈希去重
- 保留：`E:\projects\SoulMamba\soulvlm_project\temp\corpus_v3.txt`
- 禁动：`N:\ocr_datasets`
...
```text
N:\FLUED_corpus_v4\reports\FINAL_REPORT_CN.md
```

最终语料：

```text
N:\FLUED_corpus_v4\shards
N:\FLUED_corpus_v4\shards.txt
```
...
```text
E:\projects\FLUED\FLUED\tools\data\build_corpus_v4.py
```
...
```text
C:\Users\74090\AppData\Local\FLUED_corpus_v4_state\dedupe_hashes.sqlite3
```
...
```powershell
python tools\data\build_corpus_v4.py --target-gb 320 --state-dir C:\Users\74090\AppData\Local\FLUED_corpus_v4_state
```
...
```text
N:\FLUED_corpus_v4\shards\corpus_v4_00000.txt
N:\FLUED_corpus_v4\shards\corpus_v4_00001.txt
...
N:\FLUED_corpus_v4\manifests\selected_sources.csv
N:\FLUED_corpus_v4\reports\source_stats.csv
C:\Users\74090\AppData\Local\FLUED_corpus_v4_state\dedupe_hashes.sqlite3
```
