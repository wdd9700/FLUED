param(
    ...
    [string]$CodecCkpt = "K:\FLUED_archive\v31_language_codec_2m_20260702\codec_10k_utf8clean",
    [string]$OutRoot = "K:\FLUED_archive\v31_backbone_20260702",
    [int]$MaxSteps = 3000,
    [int]$BatchSize = 128,
    [int]$NumWorkers = 8
)
...
function Run-Backbone {
    ...
    $common = @(
        (Join-Path $repo "tools\analysis\train_v31_min_backbone.py"),
        "--mode", $Mode,
        "--data-path", $DataPath,
        "--out-dir", $runDir,
        "--device", "cuda",
...
python (Join-Path $repo "tools\analysis\summarize_v31_min_backbone.py") `
    $OutRoot `
