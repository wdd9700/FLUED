param(
  ...
  [string]$OutRoot = "K:\FLUED_archive\v3_segmental_workspace_20260629",
  [int]$MaxSteps = 15000,
  [int]$BatchSize = 16,
  [int]$SeqLen = 128,
  [int]$EvalBatches = 24,
...
function Run-Variant {
  ...
  python tools\analysis\train_v3_segmental_workspace_2m.py `
    --data-path $DataPath `
    --out-dir $outDir `
    --streaming-train `
    --seq-len $SeqLen `
  ...
  if (Test-Path $ckpt) {
    python tools\eval\eval_v3_segmental_workspace_roi.py `
      --ckpt $ckpt `
...
python tools\analysis\summarize_v3_segmental_workspace_matrix.py --root $OutRoot
