"""Analyze 5K FLUED v3.4 ablation curves without endpoint-only bias."""
...
def summarize_run(run: str, rows: list[dict], summary: dict) -> dict:
...
def write_table(rows: list[dict], out_dir: Path) -> None:
    fields = [
        ...
        "final_step",
        "identity_acc_at_500",
        "identity_acc_at_1000",
        "identity_acc_at_2500",
        "identity_acc_at_5000",
        "identity_acc_auc",
        "identity_acc_tail_slope_per_1k",
        "completion_mask_acc_at_500",
        "completion_mask_acc_at_1000",
        "completion_mask_acc_at_2500",
        "completion_mask_acc_at_5000",
        "completion_mask_acc_auc",
        "completion_mask_acc_tail_slope_per_1k",
        "actual_backbone_units_per_byte_at_5000",
        "backbone_padded_units_per_byte_at_5000",
        "eval_identity_acc",
...
<title>FLUED v3.4 5K Ablation Curves</title><style>
...
</style></head><body><main><h1>FLUED v3.4 · 5K 消融训练曲线</h1><div class="controls">
...
function draw(){const names=D.groups[group.value].filter(n=>D.runs[n]);const k=metric.value,n=+smooth.value;document.querySelector('#smoothLabel').textContent=`${n} (${n*20} steps)`;const series=names.map(name=>({name,data:avg(D.runs[name],n,k)}));const vals=series.flatMap(s=>s.data.map(x=>x.v)).filter(Number.isFinite),min=Math.min(...vals),max=Math.max(...vals),pad=(max-min||1)*.08;const y0=min-pad,y1=max+pad,W=1400,H=680,L=90,R=25,T=25,B=55;const X=x=>L+x/5000*(W-L-R),Y=y=>T+(y1-y)/(y1-y0)*(H-T-B);let h='';for(let i=0;i<=10;i++){let x=L+i/10*(W-L-R),v=i*500;h+=`<line class="grid" x1="${x}" y1="${T}" x2="${x}" y2="${H-B}"/><text class="tick" x="${x}" y="${H-24}" text-anchor="middle">${v}</text>`}for(let i=0;i<=8;i++){let y=T+i/8*(H-T-B),v=y1-i/8*(y1-y0);h+=`<line class="grid" x1="${L}" y1="${y}" x2="${W-R}" y2="${y}"/><text class="tick" x="${L-10}" y="${y+4}" text-anchor="end">${v.toFixed(3)}</text>`}
series.forEach((s,i)=>{const pts=s.data.map(p=>`${X(p.step)},${Y(p.v)}`).join(' ');h+=`<polyline data-name="${s.name}" class="${hidden.has(s.name)?'muted':''}" points="${pts}" fill="none" stroke="${colors[i%colors.length]}" stroke-width="2"/>`});h+=`<rect id="hit" x="${L}" y="${T}" width="${W-L-R}" height="${H-T-B}" fill="transparent"/>`;svg.innerHTML=h;legend.innerHTML='';series.forEach((s,i)=>{const b=document.createElement('button');b.textContent=s.name;b.style.borderBottom=`3px solid ${colors[i%colors.length]}`;if(hidden.has(s.name))b.className='muted';b.onclick=()=>{hidden.has(s.name)?hidden.delete(s.name):hidden.add(s.name);draw()};legend.appendChild(b)});document.querySelector('#hit').onmousemove=e=>{const rect=svg.getBoundingClientRect(),sx=(e.clientX-rect.left)/rect.width*W,step=Math.max(0,Math.min(5000,(sx-L)/(W-L-R)*5000)),lines=[`step ≈ ${Math.round(step)}`];series.filter(s=>!hidden.has(s.name)).forEach(s=>{const p=s.data.reduce((a,b)=>Math.abs(b.step-step)<Math.abs(a.step-step)?b:a);lines.push(`${s.name}: ${p.v.toFixed(4)}`)});tip.textContent=lines.join('\n');tip.style.display='block';tip.style.left=e.clientX+14+'px';tip.style.top=e.clientY+14+'px'};document.querySelector('#hit').onmouseleave=()=>tip.style.display='none'}
...
def main() -> None:
    ...
    for run_dir in sorted(path for path in root.iterdir() if path.is_dir()):
        ...
        summaries.append(summarize_run(run_dir.name, rows, json.loads(summary_path.read_text(encoding="utf-8"))))
    (out_dir / "curve_summary.json").write_text(json.dumps(summaries, ensure_ascii=False, indent=2), encoding="utf-8")
    write_table(summaries, out_dir)
