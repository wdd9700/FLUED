...
Object.keys(D.groups).filter(x=>D.groups[x].filter(n=>D.runs[n]).length>=2).forEach(x=>group.add(new Option(x,x)));['identity_acc','completion_mask_acc','completion_preserve_acc','loss','completion_masked_loss','soft_readout_units_per_byte','actual_backbone_units_per_byte','backbone_padded_units_per_byte'].forEach(x=>metric.add(new Option(x,x)));
