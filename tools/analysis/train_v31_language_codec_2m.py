"""Train a small FLUED-v3.1 language-codec prototype.

This script implements the corrected v3.1 interface boundary:

  bytes -> weak boundary proposal -> shared segment representation
        -> readout latent sequence (external backbone interface)
        -> summary latent sequence (internal encoder memory)
        -> latent+length decoder -> byte span reconstruction

It intentionally does not train a next-byte future head. The objective is a
codec objective: recover the bytes covered by each latent unit and learn a
stable byte-to-latent-to-byte path.
"""
...
class V31LanguageCodec2M(nn.Module):
    """Small codec-only FLUED-v3.1 prototype."""
    ...
    def forward(
        ...
        memory = self.causal_summary_memory(summary, seg_mask)
...
@torch.no_grad()
def evaluate(model: V31LanguageCodec2M, loader: DataLoader, args: argparse.Namespace, device: torch.device) -> Dict[str, float]:
    model.eval()
    totals: Dict[str, List[float]] = {"loss": [], "recon_acc": [], "length_acc": [], "boundary_acc": [], "units_per_byte": []}
...
def run(args: argparse.Namespace) -> Dict[str, float]:
    ...
    result = {"params": params, "steps": step, **{f"eval_{k}": v for k, v in eval_stats.items()}}
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train FLUED-v3.1 language codec prototype")
