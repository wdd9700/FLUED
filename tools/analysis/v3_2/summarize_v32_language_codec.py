"""Summarize FLUED-v3.2 language-codec training runs.

The script reads one or more run directories produced by
``train_v32_language_codec_2m.py`` and emits a markdown diagnostic report.  It
...
ACCURACY_KEYS = ("recon_acc", "masked_recon_acc", "keep_recon_acc", "length_acc", "masked_length_acc", "boundary_acc")
LOG_NUMERIC_KEYS = (
    "loss",
    "recon_loss",
    "masked_recon_loss",
...
def _metric_value(summary: Dict[str, Any], trends: Dict[str, Dict[str, Any]], key: str) -> float:
    value = _summary_eval(summary, key)
    if math.isfinite(value):
        return value
    trend = trends.get(key)
    if trend:
        return _to_float(trend.get("final"))
    return float("nan")


def _primary_recon_key(summary: Dict[str, Any], trends: Dict[str, Dict[str, Any]]) -> str:
    masked = _metric_value(summary, trends, "masked_recon_acc")
    if math.isfinite(masked):
        return "masked_recon_acc"
    return "recon_acc"
...
def _verdict(
    ...
    reasons: List[str] = []
    loss = trends["loss"]
    recon_key = _primary_recon_key(summary, trends)
    recon = _metric_value(summary, trends, recon_key)
    length = _metric_value(summary, trends, "length_acc")
    ...
    if math.isfinite(recon):
        if recon > 0.5:
            reasons.append(f"{recon_key} above threshold: {_fmt(recon)} > 0.5000")
        else:
            reasons.append(f"{recon_key} below threshold: {_fmt(recon)} <= 0.5000")
    else:
        reasons.append(f"{recon_key} unavailable")
...
def _overview_rows(analyses: Sequence[Dict[str, Any]]) -> List[List[str]]:
    ...
    for item in analyses:
        ...
        recon = _metric_value(summary, trends, recon_key)
        length = _metric_value(summary, trends, "length_acc")
        boundary = _metric_value(summary, trends, "boundary_acc")
...
def render_markdown(analyses: Sequence[Dict[str, Any]], notes: Sequence[str]) -> str:
    lines: List[str] = [
        "# FLUED v3.2 Language Codec Diagnostics",
        "",
    ]
    if notes:
        lines.append("Notes:")
    ...
    for item in analyses:
        ...
        length_eval = _metric_value(item["summary"], item["trends"], "length_acc")
        recon_eval = _metric_value(item["summary"], item["trends"], _primary_recon_key(item["summary"], item["trends"]))
