"""Train a minimal backbone over FLUED-v3.1 readout latents or raw bytes.

This is the first Goal-6 experiment.  It deliberately keeps the interface
boundary strict:

  latent mode:
    bytes -> frozen FLUED encoder -> readout latent sequence
...
from tools.analysis.train_v3_commit_controller_small import _append_jsonl, _cosine_with_warmup, _load_texts
...
def load_frozen_codec(path: str, device: torch.device) -> Tuple[V31LanguageCodec2M, Dict[str, object], Path]:
    ckpt_path = _resolve_ckpt(path)
    ckpt = torch.load(ckpt_path, map_location="cpu", weights_only=False)
    saved_args = dict(ckpt.get("args", {}))
    model = V31LanguageCodec2M(**_codec_kwargs(saved_args)).to(device)
...
def decode_readout(codec: V31LanguageCodec2M, readout: torch.Tensor, seg_mask: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
...
@torch.no_grad()
def evaluate(backbone: nn.Module, codec: V31LanguageCodec2M | None, loader: DataLoader, args: argparse.Namespace, device: torch.device) -> Dict[str, float]:
    backbone.eval()
...
def run(args: argparse.Namespace) -> Dict[str, float]:
    ...
    codec: V31LanguageCodec2M | None = None
