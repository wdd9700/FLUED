"""Case-based memory diagnostics for FLUED-v3.1 language codec.

This evaluator keeps the v3.1 interface boundary intact: the external output is
``readout`` only, while ``summary`` and ``memory`` are internal encoder state.
It probes whether that internal memory helps on repeated entities, technical
terms, identifiers, and cross-reference text where history should matter more
than in random corpus averages.
"""
...
from tools.analysis.v3_1.train_v31_language_codec_2m import CodecCollator, move_codec_batch  # noqa: E402
from tools.eval.v3_1.eval_v31_language_codec_memory_ablation import (  # noqa: E402
    DEFAULT_MODES,
    _dict_or_empty,
    _is_finite,
    _load_model,
        ...
        with torch.amp.autocast(device_type=device.type, dtype=torch.bfloat16, enabled=amp and device.type == "cuda"):
            ...
            for mode in modes:
                if mode == "full":
                    ...
                    byte_logits, length_logits, _metrics = forward_with_memory_mode(
                        model,
                        src,
                        valid,
                        seg_ids,
...
def _render_markdown(report: Mapping[str, Any]) -> str:
    lines: List[str] = []
    lines.append("# FLUED v3.1 Memory Case Diagnostics")
    lines.append("")
    ...
    lines.append(
        ...
        "`summary/memory` are internal encoder state."
    )
    lines.append("")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate v3.1 codec memory on hand-picked cases")
