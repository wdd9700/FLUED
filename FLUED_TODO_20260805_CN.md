...
7. **容量消融 v2（k=1 形态，进行中；2026-08-07 用户裁定：容量优先于读出数量）**：首臂 s21（d_backbone 1536）**零效应判负**（E39：0.2060 vs 0.2059——宽度非瓶颈；位置探针无近因梯度，瓶颈=单查询线性切片的提取上限）。四臂矩阵（各轴独立、从零消融协议、--backbone-readout final + latent 预测）：
   - s23：kda_head_v 256→512（**读出宽度+状态容量二合一轴**——读出的真实信息宽度是 4头×head_v 的原始切片，d_pack 只是其后 MLP 的空转余量）
   - s24：write_hidden 1024→2048（写入头翻译容量）
   - s25：summarizer_dit_layers 2→4
   - s26：decoder_layers 3→6
   参照系：s20（k=1 全 canonical）backbone 0.206 / direct 0.786。判读：任一轴 backbone 显著脱 0.2 平台（≥0.35 且曲线仍爬）= 瓶颈定位；全轴不动 = 单查询读出机制层面封顶，转 /n 翻页谱系（s22 为 n=4 地标）或写入头/状态聚合的结构性讨论。
8. **补证据串行队列（2026-08-07 用户授权自主排跑）**：矩阵 v2 收尾（s25/s26）后链接——**s27**（k=1 + 无状态通道：R1 在定义性形态下的正式判决格）、**s28**（/n n=8 + mlp：补全 n 响应曲线 1→4→8→23）、**s29**（k=1 seed 43 复现：给"零效应"判词提供 seed 噪声带）。随后 T4 探针读头（`train_predict_probe.py` 已就绪：冻结 checkpoint + 小读头训下一段字节，产出可与 H-Net 0.653 对表的 byte acc/BPB 上界口径）与 s23-s29 milestone 轨迹批量评测。矩阵全部落地后出总裁定包。
