...
6. **s20（在跑，当前最重要臂）**：k=1 主干（`--backbone-readout final`）+ latent 预测，20K。判读预注册（规格 §21）：接近 s17（0.84/0.80）强阳性；≥0.5 前端解锁 k=1；~0.2 则读出包容量是下一瓶颈。落地后由用户裁定主线形态，再重排容量消融/GRPO/翻页/多页需求任务。
