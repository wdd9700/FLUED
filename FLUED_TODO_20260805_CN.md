...
7. **容量消融 v2（k=1 形态，进行中）**：首臂 s21（d_backbone 1536）**零效应判负**（E39：0.2060 vs 0.2059——宽度非瓶颈；位置探针无近因梯度，瓶颈=单查询线性切片的提取上限）。**当前臂 s22：/4 翻页读出（`--backbone-readout paged --paged-reads 4`）+ mlp 主干**——子页级因果诚实，直接测状态承载深度（子页内最早段距读出 6 次写入）。判读：backbone ≥0.5 = /n 形态解锁，继续扫 n∈{2,4,8} 找边际拐点；仍贴 0.2 = 读出机制层面问题，回到写入头/状态聚合容量讨论。后续轴：d_pack 3072、写入头加宽、decoder 1.5x/2x、summarizer 重测（E22 结论域已失效）。nq 多查询同点读出轴已关闭（2026-08-07 裁定，/n 翻页为唯一合法形态）。
