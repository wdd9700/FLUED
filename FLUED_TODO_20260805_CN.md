...
- **在跑（串行，2026-08-08）**：s32（草稿臂 A：纯读出，`--backbone-mode draft
  --think-visibility readout`，`checkpoints/s32_draft_readout_20k_20260808`）→
  s33（臂 B：+memory 行，`--think-visibility readout_memory`，
  `checkpoints/s33_draft_readoutmem_20k_20260808`）。判读：对照 k=1 平台
  0.206±0.4pp 与 s31 的 0.797——臂 A 显著脱平台 = 迭代提取赌注成立（规格 §23
  隐含赌注）；臂 A vs B 的差距 = 精确查阅权值多少。早期读数：s32 @1.1K
  backbone 0.167（s20 同期 ~0.10-0.18 档），4.4 步/s，0 NaN，双臂合计 ~2.5h。
- **§23 参数已裁定回填**：T∈[1,16]、主干换 DiT 多轮去噪、任务/评测不变、
  停止机制=训练随机 T + 推理稳定停止（余弦 0.995 阈值）；因果诚实在输入层
  （预测 rollout 吃倒数第二读出+memory 截断），思考内部全局修订。
- **等用户裁定**：原路线 A/B/C 选择问题已被 §23 吸收/取代，待用户确认关闭；
  s32/s33 落地后按判读矩阵定下一步（臂 A 脱平台与否 × 两臂差距）。
