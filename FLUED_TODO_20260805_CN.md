...
| 主干 \ 状态通道 | 开 | 关 |
|---|---|---|
| attn（双向） | s16：0.832/0.785/2.30 | s18：**0.850/0.805/2.12** |
| mlp（逐点，因果诚实） | s17：**0.843/0.799/2.17，cos 0.953** | s19：在跑 |

- E32 已推翻：per-readout 不仅没死，还 ≥ E30，且是**唯一训练形态=流式生成形态**的配置。
- attn 下状态通道冗余（s18>s16）——双向注意力在时串行压缩只丢信息；状态通道的价值要看 mlp 形态（s17 vs s19，它是唯一跨段通道的地方）。
- **主线候选 = s17 形态（mlp + 状态通道 + latent 预测）**。attn 形态指标略高但双向=训练≠生成、且任何字节 CE 预测都会触发复制捷径（E34）。
- 待用户裁定：① 主线切 s17 形态（canonical backbone_mode 默认翻 mlp）；② s19 落地后若状态通道在 mlp 形态证明价值（predict_cos/masked 显著衰减若无状态），R1 判决记忆通道在部署形态下保留；③ 其后容量消融/GRPO/翻页曲线全部改基 s17 配置重排。
...
4. **R1 相对基线（健康体制）**：s16 vs s18 → attn 下无状态全胜（状态通道冗余）；正式判决格 = s17 vs s19（mlp 形态下状态通道是唯一跨段通道），s19 在跑。
