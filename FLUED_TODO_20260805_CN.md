...
7. **容量消融 v2（k=1 形态，进行中）**：瓶颈定位 512B→状态→读出 1536→**d_backbone 384 掐口**→decoder。首臂 **s21（d_backbone 1536 对齐 d_pack，decoder 加 out_proj 脱钩字节表，~115M）**在跑；后续轴按结果排：d_pack 1536→3072、写入头加宽、decoder 1.5x/2x、summarizer 重测（E22 结论域已失效）。**nq 轴已关闭（2026-08-07 用户裁定：nq>1 非 k=1 合法扩容，除非 /n 翻页，否则破坏因果；压缩率非硬瓶颈，智能密度才是——规格 §22）**。目标：backbone 脱 0.2 平台 = k=1 解锁；全轴不动 = 形态层面问题。
