...
## 0.3 进行中与下一步（2026-08-07 晚，压缩接力锚点）

- **在跑（串行）**：s30（16 头同状态尺寸：地址数 ×4 判别臂，`checkpoints/s30_k1_heads16_20k_20260807`）→ s31（因果 cross-read 主干 `--backbone-mode xattn`：Q=逐段位置、K/V=未压缩 memory、严格因果，`checkpoints/s31_xattn_20k_20260807`）。完成通知到达后读两目录的 `summary.json` 与 `train_log.jsonl`。
- **判读矩阵（E43，预注册）**：s30 脱平台→地址数主因；s31 脱平台→缺精确通道主因（混合形态成主干候选）；双不动→转 s32（迭代读出，多步取址保 k=1 字面）或 C3（摘要记录分层写入）/latent-MoE 写入头。
- **等用户裁定**：路线 A（小页 k=1）/B（/n 稠密读出）/C（写入侧结构）/新机制的选择（总裁定包已交付，用户思考中）。TERMS 口径已裁定（仅 predict_cos/predict_byte_acc 转正，其余降白话）。
- **语料线**：1TB 盘缺口清单在 `docs/data/CORPUS_GAP_AUDIT_20260807_CN.md`；W/X/Y 盘挂上后逐盘点库（v5 暂存核查）；P0 评测集（LoCoMo/LongBench 等 <10GB）可随时先建——D5 文档级考场前置。
- **档案纪律**：新归档一律 `K:\FLUED_archive\`（含 SHA256SUMS）；push 失败挂 90 秒重试循环；训练用 soulvlm 直调 + OMP/MKL=4；checkpoint 删除禁用通配符。
