...
4. **R1 相对基线（已闭环，判决暂缓触发）**：2×2 全落地（s16/s17/s18/s19）——512B 尺度状态通道无可测贡献（attn 下冗余、mlp 下噪声带内），但 ① 规格 §7 关闭判决的需求结构前置（文档级任务/长上下文）未创造；② 规格 §21 正本：2×2 测于"主干吃 23 段"的非本意形态，状态通道的定义性角色只在 k=1 接口下行使——**正式考场移交给 s20 形态**。
5. **R0 Occam 基线（已闭环，E36）**：KDA-LM 3:1 双臂 BPB 1.80/1.85，同协议大幅落后 H-Net 复现锚 0.653——处决开关未触发且方向远离，R0 线关闭；v3.6 侧因果 LM 读数待 T4 探针读头。
6. **s20（k=1 主干首臂，已闭环，E38）**：direct 0.786（−5.7pp 税）/ backbone 0.206（全文补全，**落预注册第三档：读出接口容量是下一瓶颈**）/ predict lift +0.26（首个实质预测信号）。E37（用户"分数略低上限更高"预测）：分数更低成立，上限更高在当前容量不成立（曲线 3K 起平台，接口即刻饱和）。
7. **容量消融 v2（k=1 形态，进行中）**：瓶颈定位 512B→状态→读出 1536→**d_backbone 384 掐口**→decoder。首臂 **s21（d_backbone 1536 对齐 d_pack，decoder 加 out_proj 脱钩字节表，~115M）**在跑；后续轴按结果排：d_pack 1536→3072、写入头加宽、decoder 1.5x/2x、summarizer 重测（E22 结论域已失效）。nq>1（多查询读出）涉及 k=1 口径边界，待用户裁定。目标：backbone 脱 0.2 平台 = k=1 解锁；全轴不动 = 形态层面问题。
