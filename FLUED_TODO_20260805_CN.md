...
## 0.1 基线裁定——**2×2 已基本读完，主线候选浮现（2026-08-06，待用户裁定）**

健康体制（latent 预测）2×2，其余全同 v36.7：

| 主干 \ 状态通道 | 开 | 关 |
|---|---|---|
| attn（双向） | s16：0.832/0.785/2.30 | s18：**0.850/0.805/2.12**（无状态全胜，超噪声带） |
| mlp（逐点，因果诚实） | s17：**0.843/0.799/2.17，cos 0.953** | s19：0.837/0.793/2.21，cos 0.941（差距在噪声带内） |

- E32 已推翻：per-readout 不仅没死，还 ≥ E30，且是**唯一训练形态=流式生成形态**的配置。
- R1 读数（512B/23 段/当前三任务）：attn 下状态通道冗余（只丢信息）；mlp 下增益在噪声带内——**当前尺度状态通道无可测贡献**（与 E7/E13 同签名），其真实考场在文档级需求任务与翻页后跨矩阵载体角色。规格 §7 的关闭判决**暂不触发**（需求结构未创造，规格自己的前置条件）。
- **主线候选 = s17 形态（mlp + 状态通道 + latent 预测）**：指标 ≥ E30、唯一因果诚实、状态通道在其内担任部署时唯一跨矩阵载体。attn 形态指标略高但双向=训练≠生成、且字节 CE 预测必触发复制捷径（E34）。
- 待用户裁定：① 主线切 s17 形态（canonical backbone_mode 默认翻 mlp）；② 其后容量消融/GRPO/翻页曲线全部改基 s17 配置重排。
- 判读注意：predict_cos 邻段相似底数未测，cos 小差异暂不作架构级判读（底数探针排期）。
...
4. **R1 相对基线（已闭环，判决暂缓触发）**：2×2 全落地（s16/s17/s18/s19）——512B 尺度状态通道无可测贡献（attn 下冗余、mlp 下噪声带内），但规格 §7 关闭判决的需求结构前置（文档级任务/长上下文）未创造，**暂不关闭**；其部署价值在翻页后跨矩阵载体角色。
5. **R0 Occam 基线（训练中）**：双臂 20K 在跑（A 宽浅 d512/L12/ffn1792=48.21M / B 窄深 d448/L16/ffn1536≈47.3M），同 H-Net 复现协议，BPB 口径。判决：同参打平 → v3.6 全线关闭；胜出需质量+分段计时双赢。
6. 归因链闭环后由用户裁定主线形态（候选=s17 配置），再重排容量消融/GRPO/翻页曲线。
