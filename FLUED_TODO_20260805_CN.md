...
- **当前主线（用户提案，规格 §23 候选）**：三态草稿架构——输入态=k=1 单读出
  矩阵（岗位改为"给思考提供 grist"）、思考态=新矩阵迭代去噪（步数主干定、
  min/max 兜底、**逐步不监督：输出干净思考可以脏**）、输出态=one-shot 新矩阵
  交 decoder。**评测线不动**（主干任务梯度天然约束 codec）。
- **下一个判别臂（双方保留意见，实验裁决）**：思考步可见范围——臂 A 纯读出
  （用户口径，架构干净）vs 臂 B 读出+memory 行（AI 口径，预期分高）。同预算
  同协议对判。隐含赌注：迭代去噪能否取出单趟前馈取不出的叠加信息（吸收 s32）。
- **等用户裁定**：§23 实现排期（先做哪条臂、T_min/T_max、噪声过程选型）；
